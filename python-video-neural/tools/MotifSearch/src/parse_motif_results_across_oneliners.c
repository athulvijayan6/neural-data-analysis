...
int main(int argc, char* argv[]) {
  ...
  for (i = 0; i < spot_vec_ptr -> size; i++) {
    ...
    int j;
    for (j = 0; j < ref_ctr; j++)
      if (strcmp(spot_vec_ptr->vec[i].query_path, refs[j])==0)
	break;
    assert(j < ref_ctr);
    int k;
    for (k = 0; k < ref_ctr; k++)
    ...
    fprintf(fout, "%d %d %d %d %d %d %f %f %f %f %f %f\n",
	    j,
	    k,
	    spot_vec_ptr -> vec[i].best_member_ptr -> start*196,
	    spot_vec_ptr -> vec[i].best_member_ptr -> end*196,
	    spot_vec_ptr -> vec[i].best_member_ptr -> start_q,
	    spot_vec_ptr -> vec[i].best_member_ptr -> end_q,
	    spot_vec_ptr -> vec[i].best_member_ptr -> score,
	    spot_vec_ptr -> vec[i].best_member_ptr -> cost,
	    spot_vec_ptr -> vec[i].best_member_ptr -> cost_actual,
	    spot_vec_ptr -> vec[i].best_member_ptr -> waq,
	    spot_vec_ptr -> vec[i].best_member_ptr -> war,
	    (spot_vec_ptr -> vec[i].best_member_ptr -> end*196 -  spot_vec_ptr -> vec[i].best_member_ptr -> start*196 +1)/44100.0
	    );
  }
