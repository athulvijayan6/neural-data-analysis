...
void motif_search_in_reference(const SegmentPTR query_ptr,const SegmentPTR reference_ptr, const char * ctrl_file, MemberVectorPTR member_vec_ptr) {
  ...
  for (qvi =0; qvi < query_ptr->roi_ptr->size; qvi++) {
	...
	realloc_members_MemberVectorPTR(member_vec_ptr, 2*member_vec_ptr->max_size);
      ...
      RLCS_perform(query_ptr->feat_mat_ptr, reference_ptr->feat_mat_ptr, parameters_slcs_ptr, start_q, end_q, start, end, &(member_vec_ptr -> vec[si]), 0, query_ptr->saddle_points_ptr, reference_ptr->saddle_points_ptr);
...
void motif_search_in_reference_2(const SegmentPTR query_ptr,const SegmentPTR reference_ptr, const char * ctrl_file, MemberVectorPTR member_vec_ptr) {
  ...
  SaddlePointsPTR a = create_SaddlePointsPTR();
  SaddlePointsPTR b = create_SaddlePointsPTR();

  RLCS_perform(query_ptr->feat_mat_ptr, reference_ptr->feat_mat_ptr, parameters_slcs_ptr, 0, query_ptr->feat_mat_ptr->size-1, 0, reference_ptr->feat_mat_ptr->size-1, &(member_vec_ptr -> vec[si]), 0, a, b);
  member_vec_ptr -> size += 1;
