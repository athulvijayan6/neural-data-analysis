...
int main(int argc, char* argv[]) {
  ...
  alloc_members_MemberVectorPTR(member_vec_ptr, 1);
...
void motif_search_in_reference(const SegmentPTR query_ptr,const SegmentPTR reference_ptr, const String ctrl_file, MemberVectorPTR member_vec_ptr) {
  ...
  for (vi = 0; vi < reference_ptr->roi_ptr->size;  vi++) {
    ...
    // printf("roi_len --> %d\n", end-start+1);
    //printf("query_len --> %d\n", query_ptr->feat_mat_ptr->size);
    printf(" ratio --> %lf\n", (double)(end-start+1)/(double)(query_ptr->feat_mat_ptr->size));
    RLCS_window_without_grouping(query_ptr->feat_mat_ptr, reference_ptr->feat_mat_ptr, parameters_rlcs_ptr,0, query_ptr->feat_mat_ptr->size-1, start, end, reference_ptr->saddle_points_ptr->x_ptr, parameters_rlcs_ptr->hop_size_sp, reference_ptr->path, query_ptr->path, member_vec_ptr);
  }
