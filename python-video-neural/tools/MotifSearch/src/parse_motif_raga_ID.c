...
int main(int argc, char* argv[]) {
  ...
  if (argc != 4 +1) {
    printf("!!!!!!Error!!!!!\n");
    printf("Usage: %s <query-filelist> <reference-filelist> <result_file> <top_n>\n", argv[0]);
    exit(-1);
  }
  ...
  {
    FILEPTR queryfilelist = fopen(argv[1],"r");
    FILEPTR reffilelist = fopen(argv[2],"r");

    fscanf(queryfilelist, "%d", &query_ctr);
    fscanf(reffilelist, "%d", &ref_ctr);
    
    int zi;
    for (zi = 0; zi < query_ctr; zi++)
      fscanf(queryfilelist, "%s", queries[zi]);
    for (zi = 0; zi < ref_ctr; zi++)
      fscanf(reffilelist, "%s", refs[zi]);


    fclose(reffilelist);
    fclose(queryfilelist);
