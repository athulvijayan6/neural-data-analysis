...
/*
An example on how to use RLCS Library.
...
int main(int argc, char* argv[]) {
  ...
  // TODO: write a function to create these objects instead of calling malloc.  
  ...
  //TODO: Below TODO is on old TODO. Check if freeing these things still gives error.
...
void motif_search_in_reference(const SegmentPTR query_ptr,const SegmentPTR reference_ptr, const char *  ctrl_file, SpotVectorPTR spotted_potential_motif_regions_ptr, SpotVectorPTR spotted_motifs_ptr) {
  ...
  for (ri = 0; ri < spotted_potential_motif_regions_ptr->size; ri++) {
    ...
    // Make third last param 1 if want to include the third feature.
    RLCS_window(query_ptr->feat_mat_ptr, reference_ptr->feat_mat_ptr, parameters_rlcs_ptr, start_q, end_q, start, end, reference_ptr->saddle_points_ptr->x_ptr, parameters_rlcs_ptr->hop_size_sp, reference_ptr->path, query_ptr->path, spotted_motifs_ptr, 0, query_ptr->saddle_points_ptr, reference_ptr->saddle_points_ptr);
  }
